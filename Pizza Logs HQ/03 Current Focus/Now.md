...
**Animation verification passed locally and on production.** Focused red/green tests covered the intro source, reusable reveal/order helper, Guild row wiring, and Tailwind safelist. TypeScript, ESLint, `git diff --check`, and `next build` passed. Local production CSS checks confirmed the built stylesheet now contains `.reveal-item`, `.boss-reveal-item`, both reveal keyframes, and the `420ms` timing. Earlier headless Chrome checks confirmed first-visit intro, skip, refresh suppression, auto-complete, reduced-motion behavior, and mobile `/players` overflow/search/nav behavior.
