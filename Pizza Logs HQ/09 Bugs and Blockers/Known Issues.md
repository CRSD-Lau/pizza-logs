...
| Warmane character portraits may be canvas/model-only | Exact in-game faces require Warmane/Wowhead to expose a static portrait or allow the browser userscript to export the rendered modelviewer canvas. If the canvas is cross-origin tainted, never reaches a non-black render state, or no portrait-like asset exists, Pizza Logs must fall back to class icons/initials. | Portrait Userscript `0.4.0` runs on Pizza Logs, Warmane character pages, and Wowhead/Zamimg modelviewer frames, rejects blank/black canvas captures, caches static URLs or readable rendered canvas data through Tampermonkey storage, and needs normal-browser validation after deploy/local install. |
